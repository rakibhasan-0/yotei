...
function AddActivity({ id, setShowActivityInfo, sendActivity = null, scrollId="scrollable-content" }) {
	...
	const sortOptionsExercise = [
		...
		{
			label: "Tid: Kortast först",
			cmp: (a, b) => {
				return a.duration - b.duration
			},
		},
		{
			label: "Tid: Längst först",
			cmp: (a, b) => {
	...
	useEffect(() => sessionStorage.setItem("sort", JSON.stringify(sortExercise)))
	...
	useEffect(() => {
		...
		setTechniques(
			techniques.filter((technique) =>
				checkedActivities.some((checkedActivity) => checkedActivity.techniqueID === technique.techniqueID)
			)
		)
		searchTechniques()
	...
	useEffect(() => {
		...
		setExercises(
			exercises.filter((exercise) =>
				checkedActivities.some((checkedActivity) => checkedActivity.id === exercise.id)
			)
		)
		searchExercises()
			...
			<Modal.Body style={{ padding: "0" }}>
				<Tabs activeKey={key} onSelect={(k) => setKey(k)} className={style.tabs}>
					...
					<Tab eventKey="exercise" title="Övningar" tabClassName={style.tab}>
						...
						<NewSorter
							onSortChange={setSortExercise}
							id="ei-sort"
							selected={sortExercise}
							options={sortOptionsExercise}
						/>
					...
					<Tab eventKey="lists" title="Listor" tabClassName={`nav-link ${style.tab}`}>
						<div className={style.searchBar}>
							...
							<NewSorter
								onSortChange={setSortLists}
								id="ei-sort"
								selected={sortLists}
								options={sortOptionsLists}
							/>
