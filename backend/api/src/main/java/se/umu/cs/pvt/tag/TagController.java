...
public class TagController {
    ...
    /**
     * Gets all tags, with usage statistics, sorted according to the sortBy parameter, and filtered according to contains.
     * 
     ...
     * @return The list of tags sorted and filtered tags, with the usage numbers.
     *         e.g. {"id":1,"name":"my_tag","exercises":3,"techniques":4,"workouts":0}
     */
    @GetMapping("/filter")
    public ResponseEntity<List<TagRepository.TagWithUsage>> filterTags(@RequestParam("sort-by") String sortBy, String contains) {
        if (sortBy == null) return new ResponseEntity<>(HttpStatus.BAD_REQUEST);

        List<TagRepository.TagWithUsage> result;
        ...
        if (contains != null && !(contains = contains.trim()).isEmpty()) {
            result = switch (sortBy) {
                ...
                case "name-desc" -> tagRepository.findAllByNameContainingIgnoreCaseOrderByNameDesc(EscapeCharacter.DEFAULT.escape(contains));
                case "name-asc"  -> tagRepository.findAllByNameContainingIgnoreCaseOrderByNameAsc (EscapeCharacter.DEFAULT.escape(contains));
                default -> null;
            };
        }
        else {
