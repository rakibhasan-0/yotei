...
public class ExerTagControllerTest {
    ...
    void shouldAddExerTag() {
        ...
        // save is a function called in the JPA repository when a exercise tag is added
        // via the controller
        ...
        // this line should not post anything since the tag is only posted if there
        // exists a regular tag with the correct id in the Tag table.
        assertEquals(new ResponseEntity<>(exerTag, HttpStatus.BAD_REQUEST),
                exerController.postExerciseTagPair(exerTag, regularTag.getId()));
        assertTrue(exerciseTags.isEmpty());

        // we redefine the findById method (that is called in exerciseTagController) to
        // return a tag and try posting again
    ...
    void shouldGetTagsFromExercise() {
        ...
        // findAllProjectedByExerId is the function called in the JPA repository when a
        // search for tags related to a exercise is made.
        ...
        // check that the different exercises have the correct amount of tags and a Http
        // status code is returned when trying to fetch with non existing exercise id.
    ...
    void shouldRemoveExerciseTags() {
        ...
        // should return null when attempting to remove a ExerciseTag that does not
        // exist
        assertEquals(new ResponseEntity<>(HttpStatus.BAD_REQUEST),
                exerController.deleteExerciseTagPair(new ExerciseTag((long) 32), regularTag.getId()));
    }
}