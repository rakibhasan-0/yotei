...
import { WORKOUT_CREATE_TYPES, compareCurrentToOriginal } from "./WorkoutCreateReducer"
...
function WorkoutFormComponent({callback}){
	...
	function handleSubmit(event){
		...
		else if(workoutCreateInfo.data.activityItems.length == 0 && !acceptActivities) {
	...
	return (
		<Form noValidate validated={validated} onSubmit={handleSubmit} >
			<Row style={{display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", height: "100%"}} className="Test">
				<Form.Group as={Col} md="9" controlId='validationCustom01' className='mb-1'>
					<Form.Control
						type="text"
						placeholder="Namn"
						errorMessage= {validated && workoutCreateInfo.data.name.length == 0 ? "Fyll i namn på passet" : "" }
						as={InputTextField}
						text={workoutCreateInfo.data.name}
						value={workoutCreateInfo.data.name}
						required
						onChange={e => workoutCreateInfoDispatch({type: WORKOUT_CREATE_TYPES.SET_NAME, name: e.target.value })}
					/>
				</Form.Group>

				<Form.Group as={Col} md="9" controlId='validationCustom03' className='mb-1'>
					<Form.Control
						...
						errorMessage= {validated && workoutCreateInfo.data.description.length == 0 ? "Fyll i beskrivning för passet" : "" }
						required
						onChange={e => workoutCreateInfoDispatch({type: WORKOUT_CREATE_TYPES.SET_DESCRIPTION, description: e.target.value })}
					/>
				</Form.Group>
				...
				<Form.Group as={Col} md="9" className="mb-3">
					...
					<ConfirmPopup
						...
						showPopup={validated && workoutCreateInfo.data.activityItems.length == 0 && acceptActivities} 
					...
					<div className={styles.activityButtonContainer}>
						<Button
							onClick={() => 
								workoutCreateInfoDispatch({type: WORKOUT_CREATE_TYPES.OPEN_FREE_TEXT_POPUP})
							}>
							<h2>+ Fri text</h2>
						</Button>
						<Button
							onClick={() =>
								workoutCreateInfoDispatch({type: WORKOUT_CREATE_TYPES.OPEN_CHOOSE_ACTIVITY_POPUP})
							}>
							<h2>+ Aktivitet</h2>
						</Button>
					</div>
				...
				<Form.Group as={Col} md="9" className="mb-3">
					<AddUserComponent 
						...
						setAddedUsers={(users) => workoutCreateInfoDispatch({type: WORKOUT_CREATE_TYPES.SET_USERS, users})} />
				</Form.Group>
				...
				<Form.Group as={Col} md="9" className="mb-3">
					...
					<TagInput 
						...
						setAddedTags={(tags) => workoutCreateInfoDispatch({type: WORKOUT_CREATE_TYPES.SET_TAGS, tags})} />
				</Form.Group>

				<Form.Group as={Col} md="9" className={styles.buttonContainer}>
					<Button onClick={handleGoBack} outlined={true} id="workout-create-back-button">
						<h2>Tillbaka</h2>
					</Button>
					<ConfirmPopup
